...
**使用メソッド**: `postForAuth()` - ログイン専用
**エラー時の動作**: スナックバー通知のみ表示（自動的なページ遷移なし）

- ログイン(POST /login)
- サインアップ(POST /register)

| status code | 内容                           | UI表示                     | 出力メッセージ                                    |
|-------------|--------------------------------|----------------------------|---------------------------------------------------|
| 400         | 入力内容の不正                 | スナックバー通知（3秒表示） | 処理に失敗しました。再度お試しください。          |
| 401         | 認証失敗                       | スナックバー通知（3秒表示） | ユーザー名・パスワードに誤りがあります。          |
| 404         | 入力されたユーザーが存在しない | スナックバー通知（3秒表示） | ユーザー名・パスワードに誤りがあります。          |
| 500         | サーバーエラー                 | スナックバー通知（3秒表示） | しばらくしてから再度お試しください。              |
| その他      | 予期せぬエラー                 | スナックバー通知（3秒表示） | 予期せぬエラーが発生しました。しばらくしてから再度お試しください。 |


### ホーム画面(/home)

**使用メソッド**: カスタム実装（直接fetch） - `GET /home`
**エラー時の動作**:
- トークンなし: /unauthorizedエラーページへ遷移
- 401: /unauthorizedエラーページへ遷移（スナックバー通知なし、ユーザー情報クリアなし）
- その他: コンソールログのみ出力

- ホーム画面の認証チェック(GET /home)

| status code | 内容               | UI表示                    | 出力メッセージ |
|-------------|--------------------|---------------------------|----------------|
| 401         | トークン無効       | /unauthorizedへ遷移       | （表示なし）   |
| 500         | サーバーエラー     | コンソールログ出力        | （表示なし）   |


### 単語帳画面(/book)

**使用メソッド**: `apiClient.get()` - 標準
**エラー時の動作**:
- 403: /forbiddenエラーページへ遷移（スナックバー通知なし）
- 401: スナックバー通知 → ユーザー情報クリア → /loginページへリダイレクト
- その他: スナックバー通知のみ表示
...
| status code | 内容               | UI表示                                 | 出力メッセージ                         |
|-------------|--------------------|----------------------------------------|----------------------------------------|
| 401         | トークン無効       | スナックバー通知 → /loginへリダイレクト | 有効な認証情報が見つかりませんでした。 |
| 403         | トークンの認可失敗 | /forbiddenへ遷移                        | （表示なし）                           |
| 500         | サーバーエラー     | スナックバー通知（3秒表示）             | しばらくしてから再度お試しください。   |


### 作成画面(/create)

**使用メソッド**: `apiClient.post()` - 標準
**エラー時の動作**:
- 403: /forbiddenエラーページへ遷移（スナックバー通知なし）
- 401: スナックバー通知 → ユーザー情報クリア → /loginページへリダイレクト
- その他: スナックバー通知のみ表示
...
| 401         | トークン無効                       | スナックバー通知 → /loginへリダイレクト | 有効な認証情報が見つかりませんでした。 |
| 403         | トークンの認可失敗                 | /forbiddenへ遷移                        | （表示なし）                           |
| 422         | 単語帳タイトル/単語/翻訳が長すぎる | スナックバー通知（3秒表示）             | 入力上限を超えています。               |
| 500         | サーバーエラー                     | スナックバー通知（3秒表示）             | しばらくしてから再度お試しください。   |
